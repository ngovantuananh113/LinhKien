    ...
    <div class="flex flex-col items-center justify-center min-h-[min(78vh,calc(100vh-10rem))] py-8 sm:py-12 px-4 sm:px-6">
        <div class="w-full max-w-lg">
            <header class="text-center mb-8 sm:mb-10">
                ...
                <p class="text-secondary-container font-headline text-[10px] tracking-[0.4em] uppercase font-bold mb-2">Danh mục / Chỉnh sửa</p>
                <h1 class="font-headline text-2xl sm:text-3xl md:text-4xl font-black text-on-background uppercase tracking-tight">Sửa danh mục</h1>
                <p class="text-outline mt-3 text-sm font-body max-w-md mx-auto leading-relaxed">Mã nội bộ: <span class="text-primary font-mono">#{{ $category->id }}</span> — {{ $category->name }}</p>
            </header>
                    ...
                    <form action="{{ route('admin.categories.update', $category) }}" method="post" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="w-full bg-surface-container-low border border-outline-variant/35 px-4 py-3.5 text-on-background focus:border-secondary-container focus:ring-0 focus:outline-none font-body transition-colors">{{ old('description', $category->description) }}</textarea>
                        </div>
                        <div class="flex flex-col sm:flex-row flex-wrap gap-3 pt-2">
                            <button type="submit" class="flex-1 sm:flex-none inline-flex items-center justify-center px-10 py-3.5 min-h-[48px] bg-gradient-to-r from-primary-container to-secondary-container text-white font-label text-xs font-black uppercase tracking-[0.2em] shadow-[0_0_28px_rgba(0,244,254,0.25)] hover:brightness-110 active:scale-[0.99] transition-all">Cập nhật</button>
